      ...
      <div class="relative -mt-16 h-56 w-full overflow-hidden border-b border-white/5 bg-[#0a0a0a] pt-16 sm:h-68">
        ...
        >
          <div class="text-center">
            <h1 class="mb-1 text-3xl font-extrabold tracking-tight text-white drop-shadow-md sm:text-4xl">Continue Reading</h1>
            <p class="text-xs font-medium text-gray-400 sm:text-sm">Pick up right where you left off in your manga</p>
          </div>
        </div>
      </div>
        ...
        <div class="px-4 pt-6 pb-8">
            ...
            <div class="flex flex-col items-center justify-center py-24 sm:py-32">
              <div class="animate-fade-in-up flex flex-col items-center text-center">
                ...
                <p class="mb-6 max-w-[280px] text-sm leading-relaxed font-medium text-gray-400">
                  Connect your AniList account to manage your continue reading list.
                </p>
          ...
          <template v-else>
            <div class="animate-fade-in-up mb-4 text-sm font-medium text-gray-400" style="animation-delay: 300ms;">
              <span class="font-bold text-white tabular-nums shadow-sm">{{ filteredAnime.length }}</span> manga in progress
            </div>
            ...
            <div v-else-if="filteredAnime.length === 0" class="flex flex-col items-center justify-center py-24 sm:py-32">
              <div class="animate-fade-in-up flex flex-col items-center text-center">
                ...
                <h3 class="mb-2 text-xl font-black tracking-tight text-white">No Manga Found</h3>
                <p class="mb-6 max-w-[280px] text-sm leading-relaxed font-medium text-gray-400">
                  Your continue reading list is empty. Time to start exploring!
                </p>
              </div>
            </div>
...
// ---------------------------------------------------------------------------
// Title / thumbnail helpers — kept in sync with index.vue so cards resolve
// covers and titles the same way across the app.
// ---------------------------------------------------------------------------
...
async function fetchThumbnailForManga(item) {
  ...
  try {
    ...
    // network or search failure, getThumbnail fallback handles the raw cover
  }
}

async function resolveMangaThumbnails(list) {
  const batchSize = 5
...
onMounted(async () => {
  ...
  animeList.value = mergedEntries.map(entry => {
    ...
    return {
      ...entry,
      watchUrl: isManga ? `/manga/${entry.id}/${entry.chId || prog}` : `/anime/${entry.id}/${currentEp}`,
      title: isManga ? getMangaTitle(entry.title) : entry.title,
      cover: isManga ? getThumbnail(entry) : entry.cover,
  ...
  // Resolve real cover thumbnails via the manga-vault search API, same as
  // the homepage's continue-reading section — local data only stores the
  // last-known cover, which can be stale or missing.
  const mangaEntries = mergedEntries.filter(entry => entry.type === 'manga')
  if (mangaEntries.length) {
    await resolveMangaThumbnails(mangaEntries)
    animeList.value = animeList.value.map(entry =>
      entry.mediaType === 'manga' ? { ...entry, cover: getThumbnail(entry) } : entry
    )
  }
})
